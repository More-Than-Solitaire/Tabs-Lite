...
fun TabScreen(id: Int, idIsPlaylistEntryId: Boolean = false, navigateBack: () -> Unit) {
    ...
    val tab by with(db.tabFullDao()) { ( if(idIsPlaylistEntryId) getTabFromPlaylistEntryId(id) else getTab(id)).observeAsState(Tab()) }
    ...
    TabView(tab = tab, navigateBack = navigateBack)

    //fixme: loads updated tab to database but doesn't display it till next time
